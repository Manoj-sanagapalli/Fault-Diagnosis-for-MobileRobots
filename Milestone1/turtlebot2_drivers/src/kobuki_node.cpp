...
int main(int argc, char * argv[])
{
  ...
  //removed qos profile to make recording topics easy- need to look at it in future
  ...
  auto imu_pub = node->create_publisher<sensor_msgs::msg::Imu>("imu");
  ...
  while (rclcpp::ok()) {
    ...
    g_kobuki->unlockDataAccess();
    
    int bumper =g_kobuki->getCoreSensorData().bumper;
    int wheel = g_kobuki->getCoreSensorData().wheel_drop;
    int i =g_kobuki->getCurrentData().current[0];
    int j =g_kobuki->getCurrentData().current[1];
    ...
    current_msg->left = g_kobuki->getCurrentData().current[1];
    current_msg->right = g_kobuki->getCurrentData().current[0];
    current_msg->bumper = bumper;
    current_msg->wheel_drop = wheel;
